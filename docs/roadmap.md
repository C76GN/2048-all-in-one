...
- 当前测试：`tests/gut/` 静态计数为 22 个测试脚本、183 个 `test_` 用例；由于历史上 Godot/GUT 可能写出巨大用户目录日志，默认不直接运行裸 Godot 或 GUT。
- 安全测试入口：`tools/run_gut_safe.ps1` 已提供临时用户目录、临时日志、默认用户日志增长监控、超时和日志大小上限；2026-07-16 已用当前 `godot` 命令完成完整隔离 GUT 验证，183 个测试和 1076 个断言全部通过。
...
1. 维护“安全运行 Godot/GUT”的本地脚本。
   ...
   - 当前状态：`tools/run_gut_safe.ps1` 已提供临时 user data/log 路径、超时、日志大小上限和默认日志增长上限；Godot 4.7 stable 下安全 GUT 已通过 22 个脚本、183 个测试和 1076 个断言。
