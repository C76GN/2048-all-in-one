...
- **玩法开局组装**：首帧先提交棋盘与 HUD 真值，再让可见空格按左到右、上到下的阅读顺序以约 `18ms` 错峰从 `0.72` 倍放大，越过约 `1.06` 倍后回弹到稳定尺寸；有效方块与 HUD 纸片沿同一组装语法稍后出现。演出不阻塞输入，减少动态时直接落到终态；错峰总窗口最多 `620ms`，超过 256 个可见格时直接使用稳定终态，避免大棋盘为入场演出创建无界 Tween。
