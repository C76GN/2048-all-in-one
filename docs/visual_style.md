...
当前第一套主题是 `halftone_atlas`：灰白印刷纸面、局部低对比版纹、浅色纸片棋盘、奶油/芥黄/陶土/灰青方块，以及按身份家族固定的稀疏母题。主题是长期产品卖点，不是一次性换皮；视觉资源、UI 色板和音效主题都必须能通过设置一键切换。

## 玩家身份与界面隐喻

项目的玩家身份、页面级首要对象和 Interaction Brief 统一以
[`ui_surface_intent.md`](./ui_surface_intent.md) 为准。视觉 Thesis 回答
界面“像什么”，玩家界面意图合同回答玩家“是谁、正在操作什么”。

- 玩家是无限合并实验室的规则编排者与数字印刷工坊操作者。
- 棋盘是跨页面的真实游戏物件；模式是规则样张，书签是冻结校样，
  回放是印刷过程带，主菜单是实验室总台。
- 隐喻必须改变至少一项信息组织或因果反馈，不能只给通用页面换纸色、
  加印刷偏移或套发牌动画。
- 设置、账号、错误、无障碍和危险确认继续优先清楚、可预测；不为了
  拟物化牺牲焦点、翻译、触控或 Reduced Motion。
- List、Grid、Card 和 Form 仍是允许且必要的实现机制，但它们不能先于
  页面概念决定主对象与阅读顺序。
...
1. 先让棋盘像一个印刷游戏物件。
   ...
   - 棋盘不再依赖整块深色底板；浅色纸片底板、有效格、方块和信息卡各自承担印刷纸片层级，圆角保持很小。
...
- 纸面灰白：`#e9e6dc`
- 印刷青：`#9ed2ce`
- 深墨文字/边框：`#232326`、`#302d2b`；棋盘底板使用低对比纸色，避免深色整块压住棋盘
...
- 单次 cover/reveal 动画时长应控制在约 `0.18s` 到 `0.24s`；覆盖阶段仍需清楚可见，但完整往返不能拖慢页面到达。
...
- 游戏局内不绘制整块棋盘底板：`BoardBackground` 只保留层级与生命周期锚点，间隙直接露出纸面；空格子使用低对比橄榄暖灰、细描边和微弱硬影，让每个格子与有效方块都像独立纸片。图鉴/预览等静态场景仍可按主题使用纸色底板。
- `TileDefinition.visual_family_id` 先解析 `TileVisualFamilyStyle`，同一身份的不同数值必须保持同一家族；数值只改变色阶和字号。
- 身份家族同时决定剪角、缺口、比例、描边色与一个稀疏母题：经典使用统一的轻微裁切纸片轮廓，斐波那契为不对称剪角和角落弧线，复合家族为对角剪角，卢卡斯为八边形和成对角括号，比值基础与因子分别为侧缺口、上下票口。轮廓变化应控制在单元格内，数字本身不得旋转或偏离中心。
...
- 1280x720 是基础验证尺寸；1920x1080 和窄屏需要重新检查，不允许文字重叠、裁切或溢出。模式选择在 960x540 主动切为单列，避免配置和开始操作落到首屏以下。
...
- 空书签/回放页只保留一个紧凑说明表面和可聚焦返回动作；没有选择时隐藏预览与详情占位，选中稳定业务 ID 后才展开对应纸片。游戏结束首层只显示本局结果与三个主要动作，历史、目标、seed、校验和排行资格由可聚焦“详情”动作按需展开。
...
- `CheckButton` / `CheckBox` 的持久状态主要由轨道、拇指或勾选标记表达；所在行保持安静纸面，只在 hover/focus 时局部强调，不使用导航卡式整面青色与常驻硬投影。
...
- **玩法开局组装**：首帧先提交棋盘与 HUD 真值，并保留首块可见脚点，再让可见空格按左到右、上到下的阅读顺序以约 `14ms` 错峰从 `0.72` 倍放大，轻微越界后回弹到稳定尺寸；有效方块与 HUD 纸片沿同一组装语法分阶段稍后出现。演出不阻塞输入，减少动态时直接落到终态；错峰总窗口最多 `260ms`，超过 256 个可见格时直接使用稳定终态，避免大棋盘为入场演出创建无界 Tween。
...
- 新方块出现：约 `0.14s` 到 `0.18s`，以全不透明、略放大和极轻微错版姿态落定；不再从透明小点淡入。
- 合并：约 `0.20s` 到 `0.24s`，短暂放大后回到原位，数值成长可与落定并行。
- 移动：基准 `0.18s`，使用 `QUAD OUT` 保持路径可读；质量档位只能通过统一反馈预算缩短。
...
- **按钮**：`BaseButton` 根节点只负责布局、命中、文字与焦点，scale、position 和 modulate 始终保持基础值。`GameButtonMotionPresenter` 在其内部稳定包络中绘制纸片：静止态预留约 `8px` 到 `32px` 水平 motion gutter；hover/focus 首帧换为印刷青和米白粗描边，约 `75ms` 展开并按指针接触侧轻微倾斜，再用约 `120ms` 回正；pressed 在约 `75ms` 内回缩到接近静止几何、下沉并收短硬投影，释放后依据当前 hover/focus/selected 状态恢复。状态优先级为 `disabled > pressed > selected > hover/focus > rest`，按住后移出或失焦不得提前取消 pressed；运行时改变 `disabled` 必须在下一帧清除陈旧的按压、hover 和焦点表现。
- **切换控件**：开关、分段按钮和可选卡片只沿其状态轴移动一次，轨道/纸面颜色与滑块或选中标记同步落定；selected 表示持久状态，品红 focus 边框表示当前输入位置，两者不能互相替代。
- **列表与分页**：只对本次新创建且可见的条目按阅读顺序错峰出现，单项约 `0.10s` 到 `0.18s`，相邻间隔保持很小；刷新已有数据不得让整页反复重播。拥有稳定业务 ID 的图鉴、配方、成就、档案与排行榜采用 `GFTableDataView` 投影加项目 keyed cache 原位更新，保留节点、选择和焦点；只在真正删除业务项时释放节点。超长回放列表由 `GFVirtualListBinder` 统一维护有界窗口、节点池、测量与逻辑焦点；书签等小列表继续使用普通 `GFRepeaterBinder`。结构变化后由 `GFControlFocusUtility` 重建非虚拟列表焦点顺序。跨帧分页必须保存不可变请求快照，只允许最新 generation 原子替换条目、页码和焦点；旧任务只能清理自己的临时结果。
