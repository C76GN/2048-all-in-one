...
- GUT：325 个测试全部通过，共 2061 个断言。
- 当前完整套件：`tests/gut/` 下 39 个顶层测试脚本、325 个 `test_` 用例。
...
- 退出泄漏与 `.gf/godot_exit_leak_baseline.json` 一致：`ObjectDB = 319`、`Resources = 141`、RID 类型数 `= 3`，上限为 TextureStorage 11、ShapedText 9、Font 5。GF 9.0.1 声明 732 个全局脚本类，当前项目运行时声明 199 个 `class_name`。本轮以 `TurnResult` 替换 `MoveData`，并加入确定性、回放检查点、无障碍、反馈预算与可执行验收矩阵类型，运行时类净增 14 个，完整套件扩展到 39/325；审计未发现 retained Node、Resource 增长或新增 RID 类别。基线绑定 `.gf/vendor.lock.json` 的精确 GF commit、vendor tree 和项目运行时类集合；输入集合不变时任何增长都会失败。
- 临时运行目录已在成功后自动清理。
