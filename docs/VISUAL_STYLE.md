...
- 启动背景和进度条静态参数由 `resources/themes/boot/*.tres` 的 GF Profile 声明；Boot 初始化 GF 架构前只使用无状态 `GFShaderParameterUtility` 写入参数。
...
- 按钮的 hover/focus/pressed 可通过 `asset_library/shaders/ui/button_focus_dash.gdshader` 显示移动虚线圆角描边，半径、线宽、虚线密度和速度由 UI 色板引用的 GF Profile 控制；默认态必须隐藏，避免整屏控件同时动。
...
- `test_game_theme_utility.gd` 验证主题注册表、GF settings、背景/UI/VFX Profile、庆祝主题注入、方块色阶、`SceneRouterSystem` 对 GF 场景转场服务的委托、音效主题事件 ID、`GFAudioBank` 和 `GFAudioUtility` 注册。
