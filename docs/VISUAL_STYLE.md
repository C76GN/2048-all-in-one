...
背景使用 `asset_library/shaders/background/halftone_paper_background.gdshader`。方向是灰白纸底、侧边重复印刷条纹、低对比半调网点、轻纸纹、淡棋盘纸块、细虚线网格和极慢的像素墨流。具体颜色、速度和强度由 `GameTheme` 资源写入 shader 参数，shader 文件只提供安全默认值。
...
- `test_game_theme_utility.gd` 验证主题注册表、默认主题、GF settings 接入、方块色阶覆盖、`SceneRouterSystem` 对 GF 场景转场服务的委托、音效主题事件 ID、`GFAudioBank` 内容和 `GFAudioUtility` 注册。
