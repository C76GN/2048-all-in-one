...
场景切换使用 `asset_library/shaders/transition/halftone_wipe_transition.gdshader`，由 `SceneRouterSystem` 在根视口上创建全局 `CanvasLayer` 遮罩。该 shader 来自外部 2D 遮罩转场思路，但项目版不依赖外部 gradient 或 shape texture，改为程序化斜向印刷擦除、形状遮罩推进、半调网点、轻纸纹、半透明铺墨和青/品红错版移动边。
...
- `test_game_theme_utility.gd` 验证主题注册表、默认主题、GF settings 接入、方块色阶覆盖、音效主题事件 ID、`GFAudioBank` 内容和 `GFAudioUtility` 注册。
...
- `resources/themes/game/*.tres` 是否完整引用棋盘、方块色阶、UI 色板和音效主题。
