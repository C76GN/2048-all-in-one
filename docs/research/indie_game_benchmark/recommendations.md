...
>
> **现行状态：已整体被 [`docs/roadmap.md`](../../roadmap.md) 取代。** 本文件中的 P0/P1/P2 只表示 2026-07-23 的研究排序，不是今天的优先级。涉及产品化关卡、每日/今日挑战、交互式教学或回放倍速的条目已被 [`.gf/project_contract.json`](../../../.gf/project_contract.json) 明确排除，不得据此创建任务或恢复到活跃计划。
...
- 只有已进入现行 Roadmap 的建议才可以另开项目 GitHub issue 和独立实现分支；若涉及 GF，以 issue 作为唯一协作与验收记录且不把 issue 转为 PR，再只在独立 `gf-pr` 工作区的非 `main` 分支实现。`gf-pr` 只是隔离工作区名称；完成 GF 测试、维护者发布与项目 vendor 回流前不改变本项目框架事实。用户自有 `gf` 工作区只对本项目自动化与协作者只读，不限制用户本人继续维护。本知识库本轮只保存研究证据与历史验收定义。
