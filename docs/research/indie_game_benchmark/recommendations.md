...
- 任一建议若需要游戏代码或平台服务改动，应另开 GitHub issue 和独立实现分支；若涉及 GF，以 issue 作为唯一协作与验收记录，再只在独立 `gf-pr` 工作区的非 `main` 分支实现并完成 GF 测试、发布与项目 vendor 回流，用户自有 `gf` 工作区始终只读。本知识库本轮只保存研究证据与验收定义。
