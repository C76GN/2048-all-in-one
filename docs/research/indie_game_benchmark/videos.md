...
范围：10 个由开发者或发行商上传/在官方产品页嵌入的视频，覆盖合并解谜、网格肉鸽与相邻精品独立游戏。未下载完整视频、代码或游戏素材，未运行第三方程序。
...
- 本轮共记录 **10 个官方视频、11 个时间点**，每个视频至少一个记录点直接显示实机玩法或实机场景。Dicey 的时间点精度为预览板的约 1 秒粒度，其余为播放器 `currentTime`；二者精度差异在条目中保留。
- 本轮没有独立捕获音轨，故视频条目不对音质、响度、并发或动态混音作事实判断。
...
这 10 条视频反复验证了四个可迁移模式：行动前公开后果；主目标与撤销常驻；用局部动效保持棋盘参照系稳定；让视觉风格拥有低特效/无 Shader 退路。它们应优先落为当前项目的提示、可访问性、每日挑战和统一 motion/feedback Profile，而不是复制题材、角色、卡牌、音色或素材。
