...
`GameSaveGraphUtility` 创建一个根 Scope，并由 `app/scripts/game_architecture_installer.gd` 在 GF `init()` 前登记四个 Feature section：

| Scope | Phase | Provider | Schema |
| --- | --- | --- | --- |
| `player_data` | `NORMAL` | 根作用域，无业务 Source | Profile `2` |
...
`features/persistence/scripts/data/game_save_section_data.gd` 只定义该协议。具体字段校验分别位于 `progress`、`bookmarks`、`board_editor` 和 `replays` Feature，禁止把业务 Schema 下沉到 persistence 或 shared。
...
2. 项目层校验 `player_data@2` Profile metadata。
...
- 四个 Feature section 的图健康检查与 phase。
- 统计、书签、玩家棋盘和回放只生成一个玩家数据文件。
