...
当 metadata 精确标识为同一 `player_data` schema、版本为历史正整数且低于当前版本时，启动流程先把完整已解码载荷保存到 `recovery/player_data.schema-<version>.save`，确认备份成功后再通过当前六个 section 的默认值原子重建活动文件。该流程不读取、转换或合并任何历史业务字段；备份失败时不得覆盖原活动文件。未来版本、未知 schema ID、畸形 metadata、checksum 失败和当前 section 校验失败仍直接拒绝。
...
2. 项目层校验 `player_data@4` Profile metadata；精确识别到同源旧 Profile 时执行“备份后重建”，不进入业务加载管线。
