...
本文档定义 `2048-all-in-one` 当前的玩家数据与设置持久化契约。项目使用 GF SaveGraph 统一最高分、统计、玩家棋盘、书签和回放事务；设置保持独立文件与独立生命周期。

## 设计目标

1. 玩家数据以一个原子文件保存，不出现统计成功但书签或回放失败的部分状态。
2. 每个 Feature 拥有自己的业务 Schema，持久化 Feature 不解释业务字段。
3. 加载前严格校验 Profile、Scope、Source 和 Feature Schema，未知版本直接拒绝。
4. 复用 `GFSaveGraphUtility`、`GFSaveScope`、`GFSaveDataSource` 和 `GFStorageUtility`，不在项目层重复实现对象图遍历、阶段排序、快照回滚和原子文件提交。
...
`GameSaveGraphUtility` 创建一个根 Scope，并由 `app/scripts/game_architecture_installer.gd` 在 GF `init()` 前登记五个 Feature section：

| Scope | Phase | Provider | Schema |
| --- | --- | --- | --- |
| `player_data` | `NORMAL` | 根作用域，无业务 Source | Profile `3` |
...
`features/persistence/scripts/data/game_save_section_data.gd` 只定义该协议。具体字段校验分别位于 `progress`、`bookmarks`、`board_editor`、`tile_catalog` 和 `replays` Feature，禁止把业务 Schema 下沉到 persistence 或 shared。
...
3. `GFSaveGraphUtility` 按 Scope phase 收集整张图。
4. `GFStorageUtility` 通过临时文件、事务标记和原子提交写入 `player_data.save`。
...
1. `GFStorageUtility.load_data_result()` 校验存储 envelope 与 checksum。
2. 项目层校验 `player_data@3` Profile metadata。
3. `GFSaveGraphUtility.validate_payload_for_scope(..., true)` 严格校验 Scope 和 Source 图。
4. `GFSaveGraphUtility.apply_scope(..., transactional_apply = true)` 按 `EARLY -> NORMAL -> LATE` 应用。
...
统计按稳定 `mode_id` 和 `BoardTopology.get_stable_key()` 二级 key 组织。棋盘键由语义 ID 与规范化活跃单元内容指纹共同组成，因此相同包围盒但形状不同的棋盘不会共享成绩。`SaveSystem` 负责数值归一化和统计计算，Provider 负责 section 结构边界。
...
- 当前运行时只读取当前版本，不做旧字段猜测、默认降级或双轨写入。
- 确需保留已发布版本数据时，提供一次性离线迁移工具并单独验证；迁移逻辑不得长期留在运行时主路径。
...
```powershell
powershell -ExecutionPolicy Bypass -File tools/run_gut_safe.ps1 -GodotExecutable godot -TestScripts "res://tests/gut/test_game_save_graph_utility.gd,res://tests/gut/test_save_system.gd" -TimeoutSeconds 300 -MaxLogMB 16 -MaxDefaultLogGrowthKB 128
```

回归测试必须覆盖：

- 五个 Feature section 的图健康检查与 phase。
- 统计、书签、玩家棋盘、发现进度和回放只生成一个玩家数据文件。
- Binary 往返后严格类型与稳定 UUID 保留。
- 后期 section 应用失败时早期 section 回滚。
- Profile Schema 不匹配时拒绝载荷且无旧格式回退。
- 保存失败时内存 section 回滚。
