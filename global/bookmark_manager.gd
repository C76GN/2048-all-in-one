...
const BOOKMARK_DIR = "user://bookmarks/"
...
func save_bookmark(bookmark_data: BookmarkData) -> void:
	...
	var error = ResourceSaver.save(bookmark_data, file_path)
...
func load_bookmarks() -> Array[BookmarkData]:
	...
	while file_name != "":
		if not dir.current_is_dir() and file_name.ends_with(".tres"):
			var file_path = BOOKMARK_DIR.path_join(file_name)
	...
	# 按时间戳降序排序，最新的在最前面。
	bookmarks.sort_custom(func(a, b): return a.timestamp > b.timestamp)
	return bookmarks
...
func delete_bookmark(bookmark_file_path: String) -> void:
	...
	var error = DirAccess.remove_absolute(bookmark_file_path)
	if error != OK:
