...
func test_game_initialization_records_current_session_in_level_utility() -> void:
	...
	assert_eq(GFVariantData.to_string_name(level_utility.current_level_id), &"new_game:classic_mode_config:5:12345", "当前一局应登记为稳定的 gf level session。")
	assert_eq(GFVariantData.to_string_name(level_data.get("kind")), &"2048_session", "session 元数据应标记项目层语义类型。")
	assert_eq(GFVariantData.to_string_name(level_data.get("source")), &"new_game", "普通开局应记录来源。")
	assert_eq(GFVariantData.to_text(level_data.get("mode_config_path")), _CLASSIC_MODE_CONFIG_PATH, "session 元数据应记录模式配置路径。")
	assert_eq(GFVariantData.to_int(level_data.get("grid_size")), 5, "session 元数据应记录棋盘尺寸。")
	assert_eq(GFVariantData.to_int(level_data.get("initial_seed")), 12345, "session 元数据应记录初始种子。")
