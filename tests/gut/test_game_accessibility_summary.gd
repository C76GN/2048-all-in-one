...
func test_sparse_board_summary_preserves_inactive_empty_and_tile_cells() -> void:
	...
	assert_eq(summary.kind, GameAccessibilitySummary.KIND_BOARD)
...
func test_runtime_tile_ids_do_not_change_accessibility_board_identity() -> void:
	...
	if not is_instance_valid(first_summary) or not is_instance_valid(second_summary):
		return
	assert_eq(
		first_summary.board_checksum,
		second_summary.board_checksum,
		"辅助技术使用的棋盘身份必须排除运行时 UUID。"
	)
	assert_eq(
		first_summary.canonical_payload,
		second_summary.canonical_payload,
...
func test_turn_subtitle_and_announcement_share_one_canonical_result() -> void:
	...
	if not is_instance_valid(summary):
		return
	assert_eq(summary.kind, GameAccessibilitySummary.KIND_TURN)
	...
	assert_eq(GFVariantData.get_option_int(turn_payload, &"merge_count", 0), 1)
	assert_eq(GFVariantData.get_option_int(turn_payload, &"score_delta", 0), 4)
