...
func test_pulse_presses_then_releases_exactly_once() -> void:
	...
	assert_eq(source.press_count, 1)
	assert_eq(pulse_utility.get_pending_action_count(), 1)

	await get_tree().process_frame
	await get_tree().process_frame

	assert_eq(source.release_count, 1)
	assert_eq(source.last_released_action, &"move_left")
	assert_eq(pulse_utility.get_pending_action_count(), 0)
...
func test_repeated_pulse_only_allows_latest_token_to_release() -> void:
	...
	assert_eq(source.press_count, 2)
	assert_eq(source.release_count, 1, "较早 token 不得提前释放较新的脉冲。")
	assert_eq(pulse_utility.get_pending_action_count(), 0)
...
func test_dispose_clears_source_and_late_timer_cannot_release_again() -> void:
	...
	assert_eq(source.release_count, 0, "取消后的迟到计时器不得再次写入已释放来源。")
