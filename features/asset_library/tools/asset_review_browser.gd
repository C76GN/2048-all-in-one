@tool
## AssetReviewBrowser: 项目内素材评审与实时预览工具。
class_name AssetReviewBrowser
extends Control
...
func _build_ui() -> void:
	...
	if search_input.text_changed.connect(_on_filter_changed) != OK:
		push_error("[AssetReviewBrowser] 搜索输入信号连接失败。")
	...
	if status_filter.item_selected.connect(_on_status_filter_selected) != OK:
		push_error("[AssetReviewBrowser] 状态筛选信号连接失败。")
	...
	if record_list.item_selected.connect(_on_record_selected) != OK:
		push_error("[AssetReviewBrowser] 素材列表信号连接失败。")
...
func _add_button(
	...
	if button.pressed.connect(callback) != OK:
		push_error("[AssetReviewBrowser] 按钮信号连接失败：%s" % label)
