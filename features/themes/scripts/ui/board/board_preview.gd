...
const TILE_SCENE: PackedScene = preload("res://features/themes/scenes/ui/tiles/tile.tscn")
## 用于生成预览背景格子的场景。
const GRID_CELL_SCENE: PackedScene = preload("res://features/themes/scenes/ui/board/board_grid_cell.tscn")
...
func _resolve_board_theme(mode_config: GameModeConfig) -> BoardTheme:
	...
	if is_instance_valid(theme_utility):
		return theme_utility.resolve_board_theme_for_mode(
			mode_config.visual_profile_id if is_instance_valid(mode_config) else &""
		)
	return null
...
func _resolve_color_schemes(mode_config: GameModeConfig) -> Dictionary:
	var theme_utility: GameThemeUtility = _get_theme_utility()
	if is_instance_valid(theme_utility):
		return theme_utility.resolve_color_schemes_for_mode(
			mode_config.visual_profile_id if is_instance_valid(mode_config) else &""
		)
	return {}
