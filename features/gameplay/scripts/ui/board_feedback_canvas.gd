...
func play_turn_impact(
	...
	_turn_duration = _get_turn_duration(_turn_tier)
	_turn_elapsed = 0.0
...
func play_tile_burst(
	...
	while _merge_bursts.size() >= _MAX_MERGE_BURSTS:
	...
	burst.color = color
	burst.color.a = 1.0
	burst.duration = _get_burst_duration(feedback_type)
	_merge_seed += 1
	burst.random_seed = _merge_seed
	_merge_bursts.append(burst)
	set_process(true)
	queue_redraw()
	return _get_shard_count(feedback_type) + (
...
func _draw_turn_impact() -> void:
	...
	for index: int in range(_turn_fragment_count):
		...
		var jitter: float = (_hash01(float(index * 37 + _turn_seed * 19)) - 0.5) * 18.0
		var start: Vector2 = edge_center + tangent * (normalized * span + jitter)
		var outward_distance: float = 24.0 + float(_turn_tier) * 11.0 + float(index % 4) * 6.0
...
func _draw_merge_burst(burst: MergeBurst) -> void:
	...
	var shard_count: int = _get_shard_count(burst.feedback_type)
	...
	if burst.feedback_type == &"merge" or burst.feedback_type == &"transform":
		...
		draw_arc(
			burst.position,
			lerpf(10.0, 62.0, travel_progress),
			0.0,
			TAU,
			32,
			ring_color,
	...
	for index: int in range(shard_count):
		...
		var distance: float = (22.0 + float(index % 3) * 10.0) * travel_progress
	...
	var score_center: Vector2 = (
		burst.position
		+ score_direction * score_distance * travel_progress
	)
...
func _get_turn_duration(tier: int) -> float:
	return 0.28 + float(clampi(tier, 0, 3)) * 0.035


func _get_burst_duration(feedback_type: StringName) -> float:
	match feedback_type:
		&"merge":
			return 0.46
		&"transform":
...
func _get_shard_count(feedback_type: StringName) -> int:
	match feedback_type:
		&"merge":
			return 8
		&"transform":
