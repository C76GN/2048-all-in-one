...
func _publish_gameplay_board_ready() -> void:
	...
	if is_instance_valid(game_board.model) and is_instance_valid(game_board.model.topology):
		send_event(BoardTopologyReadyData.new(game_board.model.topology))
...
func _get_game_ui_router_port() -> GameUiRouterPort:
	var utility_value: Object = get_utility(GameUiRouterPort)
