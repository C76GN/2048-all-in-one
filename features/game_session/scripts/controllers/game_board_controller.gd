...
## 预加载方块场景，用于在运行时动态实例化。
const TileScene: PackedScene = preload("res://features/themes/scenes/ui/tiles/tile.tscn")
...
## 用于生成棋盘背景格子的场景文件。
@export var grid_cell_scene: PackedScene = preload("res://features/themes/scenes/ui/board/board_grid_cell.tscn")
