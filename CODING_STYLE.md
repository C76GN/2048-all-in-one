...
*   **节与节之间**: 在不同的代码节（例如 `signal` 节和 `enum` 节，或变量区和函数区）之间，使用**一个**空行。推荐使用[节注释](#35-节注释)来标记节的开始。
*   **节内部成员的空行**:
...
### 3.3 行内注释
*   使用单井号 `#`。用于解释复杂的算法、魔法数字、或某个决策背后的原因。
*   **注释驱动分段**: 在编写函数时，可以先用注释写出逻辑步骤，然后在每个注释下填充代码。这自然地形成了逻辑块，块与块之间应留一个空行。
